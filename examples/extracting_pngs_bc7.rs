...
use wz_reader::{
    property::WzPngFormat, property::WzSubProperty, WzNode, WzNodeArc, WzNodeCast, WzObjectType,
};
...
fn main() {
    ...
    let save_image_fn = |node: &WzNodeArc| {
        ...
        if let WzObjectType::Property(WzSubProperty::PNG(png)) = &node_read.object_type {
            if png.format() == WzPngFormat::BC7 {
